...
frame:SetSize(400 * SCALE, 115 * SCALE)
frame:SetPoint("BOTTOM", 0, 250)
frame:Hide()
...
ns.showToast = function(name, icon_path, toast_type)
	...
	ticker_handler = C_Timer.NewTicker(0.01, function()
		...
		-- frame.swipe:SetAlpha(max(min(swipe_glow / 3.0, 1.0), 1.0))
		...
		if time() - start_time > TIMEOUT then
			frame:Hide()
